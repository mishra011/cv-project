...
class LoginDialog(QtGui.QDialog):
    ...
    def check(self):
        if (str(self.password.text()) == "1" and str(self.username.text()) == "1"): # do actual login check
            self.accept()
        else:
...
class MainWindow(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        layout = QVBoxLayout()
        self.btn=QPushButton("Click here to upload the video")
        ...
        self.video.setStyleSheet('background: grey')
    ...
    def getfile(self):
        ...
        def getLength(filename):
            ...
            asd = [x for x in result.stdout.readlines() if "Duration" in x]
            las = asd[0].split(",")
            las = las[0].split(":")
            dur = float(las[3]) + float(las[2])*60 + float(las[1])*60*60
            return dur
        t = getLength(fname)
        self.mplayer.loadfile(fname)
        def maino(path):
        ...
        # ====================================================
            ...
            def averageSize():
                ...
                for i in humanSizeSample:
                    sum +=i
                sampleSize = len(humanSizeSample)
        ...
        # ====================================================
            ...
            maxArea = 40000  # default max area to be considered person
            ...
            while (count < length-1):
                ...
                #qwe = (float(count/(length-1)))* 100
                qwe = int((float(count)/ float(length-1) )*100)
                
                self.progressbar.setValue(qwe)
                #print ("qwe", qwe)
                ...
                datet = "17-06-2017"
                if(time_taken > (2+rt) and time_taken <(4+rt)):
                    #print ("intervals")
                    #tList.append(rt)
                    asdf = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    asdf = asdf.split(" ")
                    asdf = list(asdf)


                    totalIn.append(peopleIn)
                    totalOut.append(peopleOut)
                    day.append(asdf[0])
                    #print (peopleIn, peopleOut)
                    peopleIn = 0
                    peopleOut = 0
                    #print ("count", count)
                    #qwe = (count/(length-1))* 100
                    #self.progressbar.setValue(qwe)
                    rt = rt + 3
                    tList2.append(asdf[1])
                else:
                    continue 
                
                    
            #print "Hello"
            log = zip(day, tList2, totalIn, totalOut)
            #print ("log", log)
            df = pd.DataFrame(log, columns = ["Date"," time","IN", "OUT"])
